...
import { Http2Fetch, Http2FetchOpts, ErrorTools, CredentialTuple } from '../http2client'
import { cortexConstants } from '../constants'
import { stringify } from 'querystring'
import { QueryServiceError } from './query_service_error'
import {
    Query, QueryJobResp, isQueryJobResp, GetJobResultsOpts,
    GetJobsListOpts, QueryJobDetail, QueryResultResp, isQueryJobDetail,
    isQueryResultResp
} from './query_service_models'
import { commonLogger, logLevel } from '../commonlogger'

const { EP_QUERY } = cortexConstants
...
export class QueryService {
    ...
    private static errorTools = new ErrorTools(QueryServiceError)
    ...
    static factory(opts: Http2FetchOpts): QueryService {
        return new QueryService(new Http2Fetch(opts))
    }
     ...
     */
    async createJob(body: Query, cred?: CredentialTuple): Promise<QueryJobResp> {
        commonLogger(logLevel.DEBUG, `createJob(). Payload: ${JSON.stringify(body)}`)
        let { data, status } = await QueryService.errorTools.tryPost(
            'ComsError', this.client,
            EP_QUERY + '/jobs', body,
            cred || this.defCred && this.defCred.cred)
        if (status == 201 && isQueryJobResp(data)) {
            return data
        }
        commonLogger(logLevel.INFO, `ParseError: status code ${status} with unparseable response: ${JSON.stringify(data)}`)
        throw new QueryServiceError('ParseError', `status code ${status} with unparseable response: ${JSON.stringify(data)}`)
    }
     ...
     * @returns the request response
     */
    async getJobResults(jobId: String, opts?: GetJobResultsOpts, cred?: CredentialTuple): Promise<QueryResultResp> {
        commonLogger(logLevel.DEBUG, `getJobResults(). jobId: ${jobId}, options: ${JSON.stringify(opts)}`)
        let { data, status } = await QueryService.errorTools.tryGet(
            'ComsError', this.client,
            QueryService.pathQsCombo(`${EP_QUERY}/jobResults/${jobId}`, opts),
            cred || this.defCred && this.defCred.cred)
        if (isQueryResultResp(data)) {
            return data
        }
        commonLogger(logLevel.INFO, `ParseError: status code ${status} with unparseable response: ${JSON.stringify(data)}`)
        throw new QueryServiceError('ParseError', `status code ${status} with unparseable response: ${JSON.stringify(data)}`)
     ...
     * @returns the request response
     */
    async getJobsList(opts: GetJobsListOpts, cred?: CredentialTuple): Promise<QueryJobDetail[]> {
        commonLogger(logLevel.DEBUG, `getJobsList(). options: ${JSON.stringify(opts)}`)
        let { data, status } = await QueryService.errorTools.tryGet(
            'ComsError', this.client,
            QueryService.pathQsCombo(`${EP_QUERY}/jobs`, opts),
            cred || this.defCred && this.defCred.cred)
        if (status == 200 && Array.isArray(data) && data.every(isQueryJobDetail)) {
            return data
        }
        commonLogger(logLevel.INFO, `ParseError: status code ${status} with unparseable response: ${JSON.stringify(data)}`)
        throw new QueryServiceError('ParseError', `status code ${status} with unparseable response: ${JSON.stringify(data)}`)
     ...
     * @returns the request response
     */
    async getJobStatus(jobId: string, cred?: CredentialTuple): Promise<QueryJobDetail> {
        commonLogger(logLevel.DEBUG, `getJobStatus(). jobId: ${jobId}`)
        let { data, status } = await QueryService.errorTools.tryGet(
            'ComsError', this.client,
            `${EP_QUERY}/jobs/${jobId}`,
            cred || this.defCred && this.defCred.cred)
        if (status == 200 && isQueryJobDetail(data)) {
            return data
        }
        commonLogger(logLevel.INFO, `ParseError: status code ${status} with unparseable response: ${JSON.stringify(data)}`)
        throw new QueryServiceError('ParseError', `status code ${status} with unparseable response: ${JSON.stringify(data)}`)
     ...
     * by the `QueryService` or its underlying `Http2Fetch` object
     */
    async deleteJob(jobId: string, cred?: CredentialTuple): Promise<void> {
        commonLogger(logLevel.DEBUG, `deleteJob(). jobId: ${jobId}`)
        let { data, status } = await QueryService.errorTools.tryDelete(
            'ComsError', this.client,
            `${EP_QUERY}/jobs/${jobId}`,
            cred || this.defCred && this.defCred.cred)
