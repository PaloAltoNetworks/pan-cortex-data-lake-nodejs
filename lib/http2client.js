...
exports.ErrorTools = exports.Http2Fetch = exports.Http2FetchError = void 0;
...
class Http2Fetch {
    ...
    constructor(opts) {
        if (!(opts && (opts.cortexDefCredentials || opts.cortexBaseFqdn))) {
            (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.INFO, `Neither 'cortexBaseFqdn' nor 'cortexDefCredentials' was provided. Will default to '${APIEPMAP['americas']}'`);
        }
        this.opts = opts;
            ...
            return new Promise((res, rej) => {
                ...
                h2session.on('connect', (sess) => {
                    h2session.setTimeout(this.cortexTimeout, () => {
                        (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.INFO, `HTTP2 session destroyed because of timeout`);
                        h2session.destroy();
                    });
                    (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.INFO, `Created new HTTP2 session to ${targetFqdn}`);
                    if (this.sessions[targetFqdn]) {
    ...
    async lazyUpdateHeaders(c) {
        if (c) {
            ...
            if (!authHeader) {
                ...
                (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.INFO, `initial autorization header for data lake id ${c.dlid}`);
                return;
            }
            ...
            if (accessToken) {
                ...
                (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.INFO, `updated autorization header for data lake id ${c.dlid}`);
            }
            return;
        }
        if (this.defaultCred) {
            ...
            if (!authHeader) {
                ...
                (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.INFO, 'initial autorization header for default data lake');
                return;
            }
            ...
            if (accessToken) {
                ...
                (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.INFO, 'updated autorization header for default data lake');
            }
            return;
        }
    ...
    async op(method, path, body, contentType, opts, cred) {
        ...
        (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.DEBUG, `[${++seqno}] http2 request operation. Method ${method}, Path: ${path}`);
        Object.entries(headers).map(x => {
            ...
            (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.DEBUG, `[${seqno}] http2 request header '${x[0]}: ${x[1]}'`);
        });
        ...
        */
        ...
        stream.on('response', headers => {
            (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.DEBUG, `[${seqno}] Response headers: ${JSON.stringify(headers)}`);
        ...
        let writePromise = (body === undefined) ? Promise.resolve() : new Promise((res, rej) => {
            try {
                stream.end(writeBuffer, () => {
                    (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.DEBUG, `[${seqno}] http2 payload write: ${writeBuffer.toString()}`);
                    res();
                });
            }
            catch (e) {
        ...
        let readPromise = new Promise((res, rej) => {
            ...
            stream.on('end', () => process.nextTick(() => {
                if (data.length == 0) {
                    (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.DEBUG, `[${seqno}] http2 read: null`);
                    return res(undefined);
                }
                (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.DEBUG, `[${seqno}] http2 read: ${data.length < 200 && data || data.slice(0, 198) + " ..."}`);
        ...
        return new Promise((res, rej) => Promise.all([readPromise, writePromise]).then(([x]) => {
            if (isNaN(status) || status >= 400) {
                (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.INFO, `Non-2xx/3xx Status Code. status: ${status}, data: ${JSON.stringify(x)}`);
    ...
    async retrier(errorType, n = 3, delay = 100, op, ...params) {
        ...
        while (a > 0) {
            try {
                ...
                (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.INFO, `retier(): Failed attempt ${a}`);
                lastError = e;
            }
