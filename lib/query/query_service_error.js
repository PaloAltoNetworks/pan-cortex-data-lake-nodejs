...
class QueryServiceError extends sdkError_1.SdkError {
    constructor(errorType, ...params) {
        ...
        let procErrors = (errors) => {
            errors.forEach(x => {
                if (query_service_models_1.isQueryApiError(x)) {
                    this.errors.push(x);
                    commonlogger_1.commonLogger(commonlogger_1.logLevel.WARNING, `QueryApi errorCode: ${x.errorCode} | message: ${x.message} | context: ${x.context} `);
                }
                else
                    commonlogger_1.commonLogger(commonlogger_1.logLevel.WARNING, `QueryApi unparseable error: ${JSON.stringify(x)}`);
            });
        };
        if (params.length > 0) {
            let p0 = params[0];
