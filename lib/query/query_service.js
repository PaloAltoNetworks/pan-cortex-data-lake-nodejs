...
class QueryService {
    static pathQsCombo(path, opts) {
        let returnPath = path;
        if (opts) {
            returnPath += '?' + (0, querystring_1.stringify)(opts);
        }
        return returnPath;
    }
    /**
    ...
    async createJob(body, cred) {
        (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.DEBUG, `createJob(). Payload: ${JSON.stringify(body)}`);
        ...
        if (status == 201 && (0, query_service_models_1.isQueryJobResp)(data)) {
            return data;
        }
        (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.INFO, `ParseError: status code ${status} with unparseable response: ${JSON.stringify(data)}`);
    ...
    async getJobResults(jobId, opts, cred) {
        (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.DEBUG, `getJobResults(). jobId: ${jobId}, options: ${JSON.stringify(opts)}`);
        ...
        if ((0, query_service_models_1.isQueryResultResp)(data)) {
            return data;
        }
        (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.INFO, `ParseError: status code ${status} with unparseable response: ${JSON.stringify(data)}`);
        throw new query_service_error_1.QueryServiceError('ParseError', `status code ${status} with unparseable response: ${JSON.stringify(data)}`);
    ...
    async getJobsList(opts, cred) {
        (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.DEBUG, `getJobsList(). options: ${JSON.stringify(opts)}`);
    ...
    async getJobStatus(jobId, cred) {
        (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.DEBUG, `getJobStatus(). jobId: ${jobId}`);
        ...
        if (status == 200 && (0, query_service_models_1.isQueryJobDetail)(data)) {
            return data;
        }
        (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.INFO, `ParseError: status code ${status} with unparseable response: ${JSON.stringify(data)}`);
        throw new query_service_error_1.QueryServiceError('ParseError', `status code ${status} with unparseable response: ${JSON.stringify(data)}`);
    ...
    async deleteJob(jobId, cred) {
        (0, commonlogger_1.commonLogger)(commonlogger_1.logLevel.DEBUG, `deleteJob(). jobId: ${jobId}`);
